...
* @parm int playerNumber
*/
void CGraphicShape::DrawSelection(C2DGraphics* renderer, int row, int col, int playerNumber) {
  ...
  //BoardPosition pos = {x+20, y+20, cellSize/2};
  
  renderer->DrawRect(x + cellSize*1/8, y + cellSize*1/8, cellSize*3/4, cellSize*3/4, Gray);
  
  
  /*           If we want to use X / O for highlights
  // Player 1 = X
  if(playerNumber == 1) {
  renderer->DrawLine(pos.x, pos.y, pos.x + pos.size, pos.y + pos.size, BRIGHT_WHITE_COLOR);
  renderer->DrawLine(pos.x + pos.size, pos.y, pos.x, pos.y + pos.size, BRIGHT_WHITE_COLOR);
  }
  // Player 2 = Circle
  else if(playerNumber == 2) {
    BoardPosition pos = {x, y, cellSize};
    renderer->DrawCircleOutline(pos.x + pos.size/2, pos.y + pos.size/2, pos.size/3, BRIGHT_WHITE_COLOR);  
  }
  */
